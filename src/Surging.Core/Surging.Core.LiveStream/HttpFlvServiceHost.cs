...
{
    public class HttpFlvServiceHost : ServiceHostAbstract
    {
        #region Field
        ...
        public HttpFlvServiceHost(Func<EndPoint, Task<IMessageListener>> messageListenerFactory) : base(null)
        {
        ...
        public override async Task StartAsync(string ip, int port)
        {
            if (_serverMessageListener != null)
                return;
            _serverMessageListener = await _messageListenerFactory(new IPEndPoint(IPAddress.Parse(ip), AppConfig.Option.HttpFlvPort));
        }
